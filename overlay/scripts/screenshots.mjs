...
await shot('panel-game', 'game', { width: 493, height: 560 }, openPanel);
await shot('panel-hover', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); await page.hover('.list-body .row:nth-of-type(4)'); });
await shot('panel-sideboard', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); await page.click('.land-row'); await page.click('.side-row'); await page.mouse.move(10, 10); });
...
await shot('panel-nobg', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); });
await shot('panel-op100', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); await page.click('button[aria-label="Settings"]'); await page.evaluate(() => { const el = document.querySelector('input[aria-label="Background opacity"]'); const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set; set.call(el, '100'); el.dispatchEvent(new Event('input', { bubbles: true })); }); await page.click('button[aria-label="Close settings"]'); });
await shot('panel-op20', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); await page.click('button[aria-label="Settings"]'); await page.evaluate(() => { const el = document.querySelector('input[aria-label="Background opacity"]'); const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set; set.call(el, '20'); el.dispatchEvent(new Event('input', { bubbles: true })); }); await page.click('button[aria-label="Close settings"]'); });
await shot('panel-nobg-flyout', 'game', { width: 493, height: 560 }, async (page) => { await board(page); await openPanel(page); await page.click('button[aria-label="Settings"]'); });
await shot('panel-scale150', 'game', { width: 740, height: 720 }, async (page) => { await board(page); await openPanel(page); await page.click('button[aria-label="Settings"]'); await page.evaluate(() => { const el = document.querySelector('input[aria-label="Scale, percent"]'); const set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set; set.call(el, '150'); el.dispatchEvent(new Event('input', { bubbles: true })); }); await page.click('button[aria-label="Close settings"]'); });
await shot('panel-all-lands', 'game', { width: 493, height: 640 }, async (page) => { await openPanel(page); await page.click('button[aria-label="Settings"]'); await page.click('text=Every land'); await page.click('button[aria-label="Close settings"]'); });
await browser.close();
server.close();
