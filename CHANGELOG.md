...
- **In-game overlay.** A small always-on-top window beside Arena: a 44 px
  ...
  when idle, steps aside when Arena isn't in front. Sits on a fixed charcoal
  tint (Background off gives text-only over the board) with hairlines
  between rows and no box border; an Opacity slider fades the whole overlay; Max panel height (share of
